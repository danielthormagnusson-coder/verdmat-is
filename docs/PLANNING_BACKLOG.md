...
### Viðauki cc117 (logged 2026-08-11) — 3a+4 í gagninu; tvennt mælt sem verður að rata í þrep 5

Þrep 3a er applýjað (`20260811_cc117_3a_flotir.sql`, 15 statement, versions `20260811222901..223048`) og þrep 4 keyrt: `scraper.auglysingamyndir` ber **988.651 raðir / 28.033 fastnúmer / 40.825 auglýsingar** og `public.property_images` ber 57 `vantar_a_r2`-merkingar á 55 fastnúmerum. Síðan er óbreytt — 3a snerti ekkert view. Þrep 1/3b/5 bíða fötuflutningsins (cc124).

- **FASTEIGNASÖLU-TAKEDOWN NÆR TIL 92,8%, EKKI 100% — OG TEXTINN TIL RÉTTHAFA VERÐUR AÐ SEGJA ÞAÐ.** `agency_name` er afritað inn á kortlagningartöfluna svo takedown sé eitt `update` en ekki join. Mæld þekja við hleðslu 11.08: **917.412 af 988.651 röðum (92,8%)**, og gatið er **ekki jafndreift**: mbl m/`listing_id` **899.599/899.603 (99,9996%)** · mbl sögulegt (án `listing_id`, þ.e. úr `listing_images.db`) **17.813/68.650 (25,9%)** · myigloo **0/20.398** (lind án miðlara — ekki gat heldur eðli). **Afritið lagar ekki 50.837 sögulegu raðirnar** — þær eiga enga auglýsingu í `scraper.listings` lengur og ekkert getur gefið þeim nafn án nýrrar heimildar; það sem afritið gerir er að **verja hinar 899.599 fyrir því að verða næsta slíkt mengi** þegar auglýsingar þeirra hverfa úr töflunni (join hefði látið þekjuna rýrna þögult með tímanum). ⇒ **KRAFA Á ÞREP 5:** `/adferdafraedi`-textinn og `/myndabeidni`-formið verða að segja að beiðni um „allar myndir frá tiltekinni fasteignasölu" nái til þess sem við getum tengt við hana, og að **beiðni sem á að ná lengra þurfi að vísa á eign eða slóð**. Loforðið má ekki vera stærra en vélbúnaðurinn ber — sama regla og felldi „tekur gildi samdægurs" úr textanum. Heimild: `scripts/hlada_auglysingamyndir.py` (hausinn á `saekja_agency`); `D:\_audit\cc117_myndbirting\`.

- **`.dbconfig` BER BOM — `utf-8-sig` ER SKILYRÐI, OG STAÐSETNING REGLUNNAR SKIPTI MÁLI.** Skráin er UTF-8 **með** BOM (`EF BB BF`); lesin með berum `utf-8` situr `﻿` fremst í URI-inu og tengingin fellur á ólæsilegri villu. Staðreyndin **stóð þegar** efst í `CLAUDE.md` (Supabase project-málsgreinin, orðrétt með `utf-8-sig`) — og **kom samt ekki í veg fyrir** að cc117 skrifaði beran `utf-8` í nýja hleðsluvél. Reglan er því nú **líka** í `## Locked operational rules`, kaflanum sem er lesinn í hverri lotu. Lærdómurinn er almennur og á við um fleiri reglur en þessa: **staðreynd í arkitektúrkafla er tilvísun, ekki vörn** — regla sem á að stöðva mistök verður að standa þar sem hún er lesin áður en kóði er skrifaður. Heimild: `CLAUDE.md` (bæði staðirnir); viðmiðunarútfærsla `scripts/myndasaekjari.py:381`.

### Viðauki cc130 (logged 2026-08-12) — ÚTDRÁTTAR-BIÐRÖÐIN FULLMÆLD; ÚTGANGURINN ER STÆRRI EN SÍAN

Read-only mæling á því hvað `--forward 200` kaupir og hver les það. Engin síun sett, ekkert þak hreyft, enginn picker snertur, engin Haiku-köll. Full úttekt (órakin, utan git): `docs/fable_prep/audits/UTDRATTAR_BIDROD_CC130_20260812T0010Z.md`. Verðeining alls staðar **$0,020717/kall** (cc127 rauðsönnun); nótt = **$4,143**. Borðið kvittaði 12.08 á alla átta liðina hér að neðan.

**NEFNARARNIR SEM ALLT ANNAÐ HVÍLIR Á (mælt á lifandi DB 12.08).** Útdrættir alls **6.272**; síðustu 30 daga **4.312 ($89,33)** á ~21,6 virkum nóttum. **Útdráttar-biðröðin er 9.112 hashar ($188,77, ~46 nætur)** — talan **20.270 úr cc128 er VERÐMATS-biðröðin** (endurmæld óbreytt í dag) og þessir tveir nefnarar hafa ítrekað verið ruglaðir saman. **Endurtekning á eign: 1.244 af 5.857 (21,2 %), þar af 25,6 % síðustu 30 daga.** **Klasa-umfram (nær-eins texti innan götu+pnr): 2.372 (39,5 %) við líkingu 0,80**, næmni 0,70 → 41,4 % · 0,90 → 34,8 % · 0,95 → 29,6 % · **0,98 → 21,7 %**.

- **(0) FORGANGSMÁLIÐ ER ÚTGANGURINN, EKKI SÍAN — SÉR LOTA NÆST, MÆLING FYRST.** Nóttin keyrir `--forward 200 --confirm --skip-valuation` og **`--bridge` er ekki í `nightly_delta_chain.sh`**. Mælt á dagatalinu: **09.08 = 200 útdrættir / 0 verðmöt · 10.08 = 200 / 0 · 11.08 = 100 / 0**. Brúin (`bridge_attributes` → `public.property_attributes`) er **eina leiðin frá útdrætti að notendafleti** — grep yfir bæði repó staðfestir að `scraper.listing_extractions` og `scraper.listing_valuations` eru **hvergi lesin af `verdmat-ai`**; `listing_valuations` birtist aðeins á `/ops` (`app/ops/page.js:242-243`). Brúin hefur keyrt **tvisvar alls**: 21 röð 03.08 (cc75-prófun) og 28.724 raðir 11.08 (handkeyrsla). **Frá 09.08 hafa 500 keypt köll (~$10,36) ekki farið í neinn sjálfvirkan feril.** Afstaða borðsins: **sía sem sparar $4/nótt er lítils virði meðan það sem er keypt kemst ekki á notendaflöt.** Umfang næstu lotu er MÆLING, ekki viðgerð: **(a)** hvers vegna er brúin opt-in — cc75 rökstuddi það á RÖÐUN (`upprunaHeiti` var switch án `default` og þekkti ekki lindina `auglysing`, svo keyrsla fyrir push hefði birt eigindi með tómri uppruna-pillu); **er sú forsenda enn til staðar í lifandi útgáfu verdmat-ai?** **(b)** hvað þarf til að hún keyri í keðjunni (hún er `try/except` og fellir aldrei nóttina, en `--bridge` í keðjunni er bókuð sem SÉR ákvörðun bæði í cc75 og í `project_components_spilling_cc94`); **(c)** hvað hefur safnast upp óbrúað síðan 11.08 og hvað af því er nýtt. **Fyrirvari sem má ekki gleymast: 47,4 %-liðurinn úr cc75** (margföldunarþak leiðréttingarlagsins fór úr 0,0 % í 47,4 % þegar brúin setti 7,17 liði á eign) **er ÓLEYST ákvörðun og hún bítur um leið og brúin verður sjálfvirk.**

- **(#2) DAUÐU KÖLLIN — RÉTTLÆTT ÁN SKILYRÐA, FER FYRST.** **271 köll / $5,62 (6,3 % næturinnar)** ná engum fleti og geta það ekki, óháð öllum öðrum ákvörðunum: **149 útdrættir á auglýsingum þar sem `fastnum` er ÓLEYST (NULL)** — `fetch_extracted_listings_to_value` krefst `fastnum IS NOT NULL` og brúin lyklar á fastnum, svo hvorug leiðin nær þeim — og **266 þar sem engin mbl-auglýsing ber textann lengur** (hann er horfinn úr `scraper.listings`). Þetta er eina mengið í allri mælingunni sem er **sannanlega** tap. Engin fórn, engin hönnun, engin skörun við hina liðina. **Hún er hrein og má fara ein.**

- **(#1) NÆR-EINS-SÍA INNAN HÚSS — RÉTTLÆTT, EN FÆR EIGIN LOTU VEGNA HÖNNUNAR.** **$32,86 / 36,8 % af 30-daga glugganum** (1.586 köll), ónæmt á þröskuldinn (21,7–41,4 %). Nákvæm samsvörun er **þegar ókeypis** — 30 hashar spanna 226 eignir á 30 köllum, hash-dedúp pickerans grípur það; vandinn er **NÆR**-eins texti sem fær aðskilinn hash og aðskilið kall. Dæmi: **Hafnarbraut 10, Kópavogi (48 íbúða nýbygging) = 64 aðskilin greidd köll, 64 eignir, textalíking 92–93 %.** **Þetta er ekki bara sparnaður heldur GÆÐABÓT og það er sterkari röksemdin:** innan 257 klasa með 3+ köllum bera **byggingarstigs**-liðirnir — staðreyndir um HÚSIÐ — ólík gildi milli íbúða í sama stigagangi: **`sameign_cosmetic` 51,0 %** · `windows_building` 42,8 % · insulation 30,0 % · cladding/foundation_drainage 26,8 % · roof 26,5 % · elevator_mechanism 23,7 %. Til samanburðar `kitchen` (íbúðarstig, MÁ vera ólíkt) 12,8 %. **Sameignin sjálf er óstöðugasti liður af öllum þrettán, óstöðugri en nokkur íbúðar-liður** — það er ekki hægt að skýra með raunmun milli íbúða, og það skrifar mótsagnir beint inn í `building_condition_score`. Hönnunarspurningin sem lotan verður að leysa: **hvernig erfist vigurinn** — byggingarstigs-reitir frá kjarna klasans, íbúðarstigs-reitir (`balcony_orientation`, `ceiling_height_premium`, fermetrar) sértækir. **`is_new_build` er ÓNOTHÆF sía á þetta: hún nær 45 af 2.372 klasa-umfram-köllum (1,9 %)** — Hafnarbraut 10, Laugavegur 105 og Hringhamar bera öll `false`; `byggar >= 2024` nær 53–61 %. **Rétta sían er nær-eins-greiningin sjálf, ekki nýbyggingarflaggið.**

- **(#3) ENDURTEKNING Á EIGN — TÍMAÞAK, EKKI BANN; BÍÐUR #1.** Tilgáta eiganda („vélin les sama skjalið aftur og aftur") **féll á nefnaranum: 62,1 % endurtekninga eru NÝR TEXTI (<60 % orðalíking)**, oftast **annar fasteignasali** á sömu eign (Fasteignaland→Domusnova, TORG→Eignamiðlun, Helgafell→Byggð); aðeins **0,7 %** eru tölubreyting ein og 18,8 % eru markup-snyrting. cc125 mældi 0 endurútdrætti á sama `source_listing_id` og sá nefnari stenst — **hash-dedúpið ver okkur gegn SAMA TEXTA, ekki gegn SÖMU EIGN**, og það eru tveir aðskildir nefnarar. Þess vegna er dómurinn tímaþak („ekki aftur á sömu eign innan n daga") en ekki bann. **Brúttó $21,42 (24,0 %) — en aðeins $6,44 (7,2 %) umfram #1.**

- **⚠ SKÖRUNARVARÚÐ — TÖLURNAR MÁ ALDREI LEGGJA SAMAN.** #1 (2.372 köll) og #3 (1.244 köll) **skarast um 897 köll**; sameiningin er **2.719 = 43,4 % allra greiddra kalla = $56,33 af $129,94**, ekki $74,91. Í 30-daga glugganum: 1.586 + 1.034 − 1.897 ⇒ **skörun 723**, svo sjálfstæð tala #3 ($21,42) er villandi og marktalan er $6,44. **Sá sem les þennan kafla og leggur saman ofmetur sparnaðinn um 33 %.** Röðin sem af þessu leiðir er bindandi: **#1 fyrst, #3 endurmælt á eftir** — aldrei öfugt og aldrei samtímis.

- **(#4) `category IN (commercial, plot, other)` — JÁ, SÍA. AFSTAÐA BORÐSINS: `/eign` Á ATVINNUHÚSNÆÐI OG LÓÐUM ER EKKI AFURÐ.** **347 köll / $7,19 / 8,0 %** í 30-daga glugganum (264 commercial · 63 plot · 20 other); **319 af 347 bera enga spá og lenda öll í `EXCLUDE`**, hin 28 falla á íbúðarflokka og eru mistalning í `category`, ekki atvinnuhúsnæði. **Athugið að þetta er STEFNUÁKVÖRÐUN sem mælingin sker ekki úr um** — hún mælir stærðina og staðfestir að spá er engin, en „er þetta afurð?" er borðsins að svara og var svarað hér. **`er_atvinnuhusnaedi` er ÓNOTHÆFUR dálkur í þessa síu: hann er RENT-ONLY** — `TRUE` á 372 leigu-auglýsingum og **`NULL` á öllum 37.236 sölu-auglýsingunum**. `category='commercial'` er eina nothæfa mælingin (2.646 sölu-auglýsingar). Í biðröðinni: commercial 895 · plot 264 · other 62.

- **(#5) EXCLUDE Í HEILD — NEI, MÆLINGIN FELLDI TILGÁTUNA.** cc116-talan stenst (**0 af 56.958 EXCLUDE-eignum bera spá í `public.predictions`**, staðfest 12.08) **en ályktunin „enginn les þetta" gerir það ekki**: brúin skrifaði **1.225 eigindaraðir á 379 EXCLUDE-eignir** og `/eign` fellur ekki á spáleysi — `lib/eign-queries.js:547` skilar `t5_astaeda = "engin_spa"` og síðan birtist áfram, svo **eigindin sjást**. EXCLUDE er flokkur sem **verðmatsleiðin má ekki lesa en eigindaleiðin les**. $7,81 sparnaður hefði kostað 379 eignir í eigindalaginu. **Almenna reglan sem þetta staðfestir enn og aftur: þekja á EINU yfirborði er ekki þekja á öllum** — sbr. `feedback_vidmid_a_eign_ekki_a_rod`.

- **NÝR LIÐUR ÚR #5: NÆTUR-VÉLIN FRYSTIR VERÐMÖT Á FLOKKI SEM BIRTINGARLEIÐIN ÚTILOKAR MEÐ ÁSETNINGI (ómælt sem villa, ekkert go).** `scraper.listing_valuations` ber **2.372 frystar raðir á 410 EXCLUDE-eignum**, meðal-`expected_base` **67,2 M kr**, meðal-gap `expected_extraction − expected_base` **−1,70 M kr**. `public.predictions` ber **núll** EXCLUDE-raðir — það er ásetningur (regla R / cc76-cc78 flokkunin) og hefur verið það lengi. **Vélarnar tvær eru því ósammála um hvaða flokk má skora**, og sú sem er ósýnileg notandanum er sú sem skorar. Þrennt þarf að skera úr: (a) er skorunin á EXCLUDE merkingarlaus (líkanið sá aldrei þann flokk í þjálfun) eða aðeins óbirt; (b) mengar hún eitthvað sem les `listing_valuations` — `/ops`, `scraper.v_expected_vs_real`, gæðaskýrslur; (c) á `value_listings` að bera sömu útilokun og birtingarleiðin. **Read-only fyrsta skref, engin skrif.** Sbr. `feedback_thjalfunarthekja_er_ekki_utgafuthekja`.

- **(#6) LANDSBYGGÐ (`region_tier = Country`) — NEI.** $23,58 / 26,4 % (1.138 köll) og biðröðin 2.828 (31,0 %), svo liðurinn er **stór** — en **engin notkunar-mæling styður síuna**: `Country`-eignir bera spá og eigindi eins og aðrar og birtast á `/eign` og `/leit` án undantekningar. Þetta væri **þekjuskerðing án mælds mótvægis**, og tilgáta eiganda um að „enginn notendaflötur biðji um landsbyggðina" er felld af mælingunni. Sama gildir um `SUMMERHOUSE` sem sérflokk (204 köll / $4,23 / 4,9 %): **89,7 % sumarhúsa-eigna með útdrátt bera `auglysing`-eigindi** — hæsta hlutfall allra flokka.

- **(#7) LEIGA (`tenure = rent`) — NEI, UNDIR SUÐMÖRKUM.** 42 köll / **$0,87 / 1,0 %** í 30-daga glugganum. Liðurinn er of lítill til að réttlæta eigin síu og eigin bilanaflöt; leigan hefur auk þess eigin notendaflöt síðan cc107 (`/leiga/[id]`). Í biðröðinni er hlutfallið hærra (461 = 5,1 %) og ef það heldur áfram að vaxa má endurmeta — **á mældri tölu, ekki á þessari.**

- **FYRIRVARI SEM MÁ EKKI TÝNAST ÚR NEINUM ÞESSARA LIÐA: SUÐGÓLFIÐ ER ÓMÆLT.** Hversu mikið eiginleikavigurinn hreyfist þegar SAMI texti er dreginn út tvisvar var ekki mælt — það krefðist Haiku-kalla sem voru bönnuð í cc130. Þess vegna er niðurstaðan „miðgildi **13 breyttir reitir af 132** milli útdrátta á sömu eign" **EFRI MÖRK á upplýsingu, ekki mæling á henni**; cc75 bókaði þegar að gólfið er ekki núll (sami prompt/skema/módel gaf 17 gegn 15 eigindum). Byggingarstigs-óstöðugleikinn í #1 (51,0 % á `sameign_cosmetic`) er **næsta besta nálgunin á gólfið og hún er há** — hún bendir til að stór hluti þessara 13 reita sé suð en ekki upplýsing. **Hver sá sem vitnar í 13-töluna sem rök FYRIR endurtekningu verður að bera fyrirvarann með.** Aðrar takmarkanir mælingarinnar: klösun er **neðri mörk** (græðg úthlutun, og klasar bundnir við sömu götu+póstnúmer svo sameiginlegt söluyfirlit yfir fleiri götur telst ekki með), og `extraction_cost_state.json` er **ekki heimild** um sögulegan kostnað (tölur fyrir 12.08 eru bókaðar á 0,0071 og eru ~2,8× of lágar, cc127) — allur kostnaður hér er reiknaður sem *köll × $0,020717*, ekki lesinn af skránni.

### Viðauki cc134 (logged 2026-08-12) — EXCLUDE ÚT ÚR VERÐMATSLEIÐINNI; FJÓRIR LIÐIR SEM MÆLINGIN FANN OG LEYSTI EKKI

Beint framhald af **NÝJUM LIÐ ÚR #5** í cc130-viðaukanum hér að ofan. Sá liður er **LOKAÐUR**: borðið kvittaði 12.08 á **(a)+(c)**, `(b)` felld. Ákvörðunin er bókuð í `docs/DECISIONS.md` §5D-2; full úttekt: `docs/fable_prep/audits/EXCLUDE_VERDMATSLEID_CC134_20260812T0006Z.md` (committuð með þessari bókun). Read-only mæling, 0 Haiku-köll, $0,00.

**Það sem er GERT:** `AND pr.canonical_code <> 'EXCLUDE'` í `fetch_extracted_listings_to_value` (`scripts/extraction_engine.py`) — biðröð 20.270 → 18.177, sannreynt gegnum raunverulega fallið. **Það sem BÍÐUR framkvæmdar:** `supabase/migrations/20260812002226_cc134_exclude_utilokun_maelifleti.sql` er **skrifuð en ÓAPPLÝJUÐ**; hún verður að fara inn áður en `/ops`-teljararnir eru lesnir aftur, annars ber `backlog.unprocessed` **varanlegt gólf 109** sem enginn getur unnið niður. Þegar hún er applýjuð: **endurmæla teljarana sjö** gegn töflunni í DECISIONS §5D-2 §4 — ekki treysta á að þeir hafi lent rétt.

- **(A) `APT_SENIOR` — 89 BIRTAR SPÁR Á FLOKKI SEM LÍKANIÐ VAR ALDREI ÞJÁLFAÐ Á. SAMA BILUN OG EXCLUDE, EN ÞEGAR KOMIN Á BIRTINGARLEIÐINA.** `public.properties` ber **14** `canonical_code`-gildi; `training_data_v2.pkl` ber **12**. Umframgildin eru `EXCLUDE` (56.958 eignir, útilokaðar með ásetningi) og **`APT_SENIOR` (89 eignir), sem er hvergi útilokaður og ber spá á ÖLLUM 89** (meðal `real_pred_mean` 57,20 M kr). Við skorun varpast hann í `NaN` nákvæmlega eins og `EXCLUDE` (`pd.Categorical` með 12 flokka korti, `phase_d3_score_extract.py:263`). **Sannprófað en ekki ályktað: adapterinn skoraði eignirnar 89 og var borinn saman við birtu töfluna — max |Δ| = 0 kr, 89/89.** Birtu spárnar voru því raunverulega reiknaðar með `canonical_code = NaN`. **Þetta fellir einföldunina sem allt cc134-verkið hvíldi á framan af — „birtingarleiðin útilokar það sem líkanið kann ekki":** hún útilokar `EXCLUDE`, en ekki `APT_SENIOR`. Umfang næstu lotu er MÆLING: **(a)** hvaðan kemur `APT_SENIOR` í flokkarann og hvenær bættist hann við (er hann yngri en þjálfunarrammi 04.08?); **(b)** hversu langt frá er talan — sama próf og cc134 keyrði á EXCLUDE (þvinga í nágrannaflokk, mæla |Δ|), sem er ódýrt og afgerandi; **(c)** hvaða flatir birta þessar 89 (`/eign`, `/leit` `er_metanleg`, comps sem nágrannar?); **(d)** er rétta viðbragðið að bæta flokknum í næsta þjálfunarramma, varpa honum á nágrannaflokk, eða útiloka hann eins og `EXCLUDE` — **það er stefnuspurning, ekki mælingarniðurstaða.** **Almenna reglan sem á að falla út úr lotunni: flokkabreyta sem er lesin af töflu en kóðuð eftir þjálfunarramma þarf HLIÐ á misræmi milli mengjanna tveggja** — í dag fellur nýtt gildi í `NaN` í fullkominni þögn. Sbr. `feedback_afleiddur_eiginleiki_ma_ekki_lesast_af_toflunni` og `feedback_thjalfunarthekja_er_ekki_utgafuthekja`.

- **(B) 531 EIGN / 2.716 RAÐIR SEM ERU ÍBÚÐARHÆFAR EN BERA SAMT ENGA SPÁ — HALINN SEM S4 HEFÐI TEKIÐ.** cc134 mældi fjórar mögulegar síur á verðmats-biðröðina; borðið valdi **S1** (`canonical_code <> 'EXCLUDE'`, 2.093 raðir burt). **S4** — „verðmeta ekkert sem birtingarleiðin ber ekki spá á" (`EXISTS public.predictions`) — hefði tekið **4.809 raðir, þ.e. 2.716 til viðbótar á 531 eign.** Þær eru **ekki** EXCLUDE: þær eru íbúðarhæfar eignir sem spátaflan sleppir samt. Per flokk, hlutfall alls safnsins án spár: **SUMMERHOUSE 16,18 % (2.253 af 13.924)** · **SFH_DETACHED 8,33 % (3.274)** · SEMI_DETACHED 3,02 % · APT_FLOOR 2,47 % (2.376) · ROW_HOUSE 2,36 %. Alls **8.426 eignir utan EXCLUDE bera enga spá.** S4 var **ekki valin** af tveimur ástæðum sem báðar standa: hún er hlið á **afurð annarrar vélar** (`public.predictions` er endurbyggð við hvert flipp, svo biðröðin myndi hreyfast við hverja endurbyggingu), og halinn sjálfur er **ómældur** — cc134 mældi stærð hans en ekki orsök. Umfang: **hvers vegna** falla þessar 8.426 út (`byggar` vantar? `matsvaedi_confident` fellur? — það eru einmitt hin tvö hliðin í trekt `phase_d3_score_extract.main()` sem `score()` ber ekki heldur), og **á verðmatsleiðin að bera þau líka**. Athugið að þetta er sama trekt og cc134 opnaði: **cc134 setti EINA af þremur hliðum inn í vélina; hin tvö standa enn utan hennar.**

- **(C) `/ops` LYKLAR BACKLOG Á AUGLÝSINGAFLOKK EN VERÐMATIÐ Á EIGNAFLOKK — TVÆR SKILGREININGAR Á „ÍBÚГ Á SAMA SPJALDI.** `ops_scraper_signals().backlog` síar á `scraper.listings_canonical.category = 'residential'` meðan verðmats-biðröðin ræðst af `public.properties.canonical_code`. Mæld afleiðing: **109 EXCLUDE-raðir í backloginu gegn 2.093 í verðmats-biðröðinni** — sami raunveruleiki, tuttugufaldur munur, af því að flokkarnir tveir mæla ekki það sama. cc134 (c) gerir teljarana **innbyrðis samkvæma** (öll sjö á eignaflokknum, svo `live_res_sale = valued + unprocessed` haldi) en **lagar ekki lyklunina sjálfa**: spjaldið segir áfram „virkar íbúða-söluauglýsingar" um mengi sem er skilgreint af auglýsingunni. Umfang: **hvor flokkurinn á að ráða á /ops**, og hvað `category` mælir í raun (cc130 fann þegar að **28 af 347 `commercial`-röðum falla á íbúðarflokka og eru mistalning**, og að `er_atvinnuhusnaedi` er RENT-ONLY dálkur). **Lítið mál í krónum, en það er beint í rekstrarspjaldinu sem á að segja satt um hvað er ógert.**

- **(D) `v_expected_vs_real` TELUR Á AUGLÝSINGU, EKKI Á EIGN — 5,25 RAÐIR PER EIGN.** 23.605 raðir dreifast á **4.494 fastnúmer**. Í seldu mengi cc134 (6 EXCLUDE-raðir) reyndust **fimm af sex vera TVÆR eignir**: Bugðufljót 9 birtist **þrisvar** og Brúarfljót 3 **tvisvar**. Eign sem er endurskráð oft vegur því margfalt í hverri samtölu sem er lesin af sýninni. cc134 mældi BÁÐA nefnara hlið við hlið og áhrifin stóðu í bæði skiptin (bjagi +2,16 pp á auglýsingastigi, +2,30 pp á eignastigi), svo **niðurstaða cc134 er ekki háð þessu** — en það er heppni þessarar mælingar, ekki eiginleiki sýnarinnar. Umfang: **á `v_expected_vs_real` að bera eignastigs-hlið** (t.d. `DISTINCT ON (fastnum)` með nýjustu frystingu) **eða á nefnarinn að vera valinn meðvitað í hverri mælingu**. Sbr. `feedback_vidmid_a_eign_ekki_a_rod` — nákvæmlega sama gerð, á nýjum fleti.
